...
struct Handler<T> {
    ...
    function: RefCell<Option<Box<dyn FnMut(T)>>>,
    #[expect(clippy::type_complexity)]
    replacement: RefCell<Option<Option<Box<dyn FnMut(T)>>>>,
}
struct HandlerRef<'a, T> {
    #[expect(clippy::type_complexity)]
    function: RefMut<'a, Option<Box<dyn FnMut(T)>>>,
    #[expect(clippy::type_complexity)]
    replacement: &'a RefCell<Option<Option<Box<dyn FnMut(T)>>>>,
}
impl<T> Handler<T> {
    fn new() -> Self {
        Self {
...
impl<T> Deref for HandlerRef<'_, T> {
    type Target = Option<Box<dyn FnMut(T)>>;
...
impl WebSocketClient {
    pub fn new(url: &str, init_message: Option<String>) -> Result<Self, JsValue> {
        ...
        let on_message = Rc::new(Handler::new());
        let on_error = Rc::new(Handler::new());
